...
{
  const form = document.getElementById('subjectForm');                              /*Aca es donde se analiza la informacion recibida, si te llego un id y un nombre, se +*/
  form.addEventListener('submit', async e =>                                   /*+ actualiza el dato ya existente, si no te llega id, significa que eso es un dato nuevo y hay que crearlo*/
  {
        e.preventDefault();
        const subject = 
        {
...
/*
fetch("backend/controllers/subjectsController.php", {
    method: "POST",
    headers: {
        "Content-Type": "application/json"
    },
    body: JSON.stringify({ name: nombreMateria })
})
.then(async response => {
    ...
    if (response.ok) {
        ...
        alert('No se puede crear la materia porque ya existe.');
    }
})*/